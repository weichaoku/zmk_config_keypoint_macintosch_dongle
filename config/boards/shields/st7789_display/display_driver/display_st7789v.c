...
static void st7789v_set_lcd_margins(const struct device *dev, uint16_t x_offset,
                                    uint16_t y_offset) {
...
static int st7789v_write(const struct device *dev, const uint16_t x, const uint16_t y,
    ...
    for (uint16_t write_cnt = 0U; write_cnt < nbr_of_writes; ++write_cnt) {
        st7789v_transmit(dev, write_cnt == 0U ? ST7789V_CMD_RAMWR : ST7789V_CMD_NONE,
                         (void *)write_data_start, desc->width * ST7789V_PIXEL_SIZE * write_h);
...
static int st7789v_set_orientation(const struct device *dev,
    ...
    uint8_t tx_data =
        config->mdac & (ST7789V_MADCTL_ML | ST7789V_MADCTL_BGR | ST7789V_MADCTL_MH_RIGHT_TO_LEFT);
...
#define ST7789V_INIT(inst)                                                                         \
    ...
    PM_DEVICE_DT_INST_DEFINE(inst, st7789v_pm_action);                                             \
                                                                                                   \
    DEVICE_DT_INST_DEFINE(inst, &st7789v_init, PM_DEVICE_DT_INST_GET(inst), &st7789v_data_##inst,  \
                          &st7789v_config_##inst, POST_KERNEL, CONFIG_DISPLAY_INIT_PRIORITY,       \
                          &st7789v_api);
